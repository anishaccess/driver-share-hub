    ...
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-50 border-b border-border/50 bg-card/80 backdrop-blur-xl">
        <div className="container mx-auto flex items-center justify-between py-4 px-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/browse">
              <Button variant="ghost" size="sm" className="font-semibold">Find Truck Owners</Button>
            </Link>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
          ...
          <div className="relative flex items-start gap-5">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-accent/20 text-accent border border-accent/20">Driver</span>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { icon: Briefcase, value: profile.experience ?? "N/A", label: "Experience", color: "text-accent" },
            { icon: Award, value: profile.rating, label: "Rating", color: "text-primary" },
            { icon: Unlock, value: unlockBalance?.remaining_unlocks ?? 0, label: "Remaining Unlocks", color: "text-primary" },
            { icon: Truck, value: recentUnlocks?.length ?? 0, label: "Owners Found", color: "text-accent" },
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-8">
          <Link to="/browse" className="group rounded-2xl border border-border/50 bg-card p-6 hover:elevated-shadow transition-all hover:-translate-y-0.5">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <h3 className="font-display font-bold text-card-foreground">Find Truck Owners</h3>
        ...
        <div className="rounded-2xl border border-border/50 bg-card p-7 mb-8 premium-shadow">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h2 className="font-display text-lg font-bold text-card-foreground">Your Profile</h2>
          </div>
          <div className="grid sm:grid-cols-3 gap-4">
            {[
              { label: "Preferred Vehicle", value: profile.vehicle_type ?? "Not set" },
              { label: "Experience", value: profile.experience ?? "Not set" },
              { label: "Profile Visibility", value: "Active ✅" },
            ].map(({ label, value }) => (
        ...
        <div className="rounded-2xl border border-border/50 bg-card p-7 premium-shadow">
          <h2 className="font-display text-lg font-bold text-card-foreground mb-5">Recently Unlocked Owners</h2>
          {recentUnlocks && recentUnlocks.length > 0 ? (
            ...
            <p className="text-sm text-muted-foreground text-center py-8">No owners unlocked yet. Start browsing!</p>
          )}
        </div>
      </div>
    </div>
