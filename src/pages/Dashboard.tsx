...
import { Truck, Users, Lock, Unlock, LogOut, MapPin, Star, Phone, Edit, ArrowRight } from "lucide-react";
...
const Dashboard = () => {
  ...
  const { data: recentUnlocks } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("*, profiles:unlocked_profile_id(full_name, role, city, phone, avatar_emoji)")
  ...
  const Icon = isOwner ? Truck : Users;
  const roleLabel = isOwner ? "Truck Owner" : "Driver";
  const browseLabel = isOwner ? "Find Drivers" : "Find Truck Owners";
      ...
      <nav className="border-b bg-card">
        <div className="container mx-auto flex items-center justify-between py-3 px-4">
          ...
          <div className="flex items-center gap-3">
            <Link to="/browse">
              <Button variant="ghost" size="sm">{browseLabel}</Button>
            </Link>
      ...
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div className="rounded-xl border bg-card p-6 mb-6">
          <div className="flex items-start gap-4">
            <div className="h-16 w-16 shrink-0 rounded-full bg-secondary flex items-center justify-center">
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${isOwner ? "bg-primary/10 text-primary" : "bg-accent/20 text-accent"}`}>
                  {roleLabel}
                </span>
              </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
          ...
          <div className="rounded-lg border bg-card p-4 text-center col-span-2 sm:col-span-1">
            ...
            <p className="text-xs text-muted-foreground">Contacts Unlocked</p>
          </div>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          <Link to="/browse" className="rounded-lg border bg-card p-5 hover:shadow-md transition-shadow">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-slab font-semibold text-card-foreground">{browseLabel}</h3>
                <p className="text-xs text-muted-foreground">Browse available profiles</p>
              </div>
        ...
        <div className="rounded-xl border bg-card p-6">
          <h2 className="font-slab text-lg font-bold text-card-foreground mb-4">Recent Unlocks</h2>
              ...
              {recentUnlocks.map((unlock: any) => (
                <div key={unlock.id} className="flex items-center gap-3 p-3 rounded-lg bg-secondary/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">{unlock.profiles?.city} · {unlock.profiles?.role}</p>
                  </div>
                  <p className="text-xs text-muted-foreground">{unlock.profiles?.phone}</p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-muted-foreground text-center py-4">No contacts unlocked yet. Start browsing!</p>
          )}
        </div>
      </div>
    </div>
